...
public class ClientProxy extends CommonProxy
{
    ...
    public void init()
    {
        ...
        {
            new OnePointSevenDelayHandler(onePointSevenDelayModule);
        }

        AutoPotModule autoPotModule = moduleManager.getModule(AutoPotModule.class);
        if (autoPotModule != null)
        {
            autoPotHandler = new AutoPotHandler(autoPotModule);
        }

        FMLCommonHandler.instance().bus().register(new GuiToggleHandler(moduleManager, configManager, autoPotHandler));

        ReachModule reachModule = moduleManager.getModule(ReachModule.class);
        if (reachModule != null)
        {
            FMLCommonHandler.instance().bus().register(reachModule);
            ChudWare.EVENT_MANAGER.register(reachModule);
        }
