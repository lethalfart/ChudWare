...
public class PacketEventBridge
{
    ...
    private void inject(Channel channel)
    {
        try
        {
            ...
            channel.pipeline().addBefore("packet_handler", HANDLER_NAME, new ChannelDuplexHandler()
            {
                ...
                {
                    if (!ChudWare.EVENT_MANAGER.hasListeners(PacketReceiveEvent.class))
                    {
                ...
                {
                    boolean hasSendListeners = ChudWare.EVENT_MANAGER.hasListeners(PacketSendEvent.class);
                    boolean hasSentListeners = ChudWare.EVENT_MANAGER.hasListeners(PacketSentEvent.class);
                    if (!hasSendListeners && !hasSentListeners)
                    {
    ...
    private boolean hasActivePacketListeners()
    {
        return ChudWare.EVENT_MANAGER.hasListeners(PacketReceiveEvent.class)
                || ChudWare.EVENT_MANAGER.hasListeners(PacketSendEvent.class)
                || ChudWare.EVENT_MANAGER.hasListeners(PacketSentEvent.class);
    }
}
